    ...
    return (
        <div className="container mx-auto p-4">
            ...
            <div className="flex justify-between items-center mb-4 bg-white p-4 rounded-lg shadow">
                ...
                <div>
                    <button onClick={() => setShowImageModal(true)} className="bg-green-500 text-white px-4 py-2 rounded mr-2 hover:bg-green-600">Upload Image</button>
                    <button onClick={() => setShowFolderModal(true)} className="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">Create Folder</button>
                </div>
            </div>
                        ...
                        {path.map((p, index) => (
                            <li key={p._id} className="inline-flex items-center">
                                <button onClick={() => handleBreadcrumbClick(index)} className="inline-flex items-center text-sm font-medium text-gray-700 hover:text-blue-600">
                                    {index > 0 && ( <svg className="w-6 h-6 text-gray-400" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clipRule="evenodd"></path></svg> )}
                                    {p.name}
                                </button>
                            </li>
                        ))}
            ...
            {loading ? <Spinner /> : (
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                    ...
                    {!searchResults && itemsToDisplay.subFolders.map(folder => (
                        <div key={folder._id} onClick={() => handleFolderClick(folder)} className="p-4 bg-white rounded-lg shadow cursor-pointer text-center hover:bg-gray-50">
                            <svg className="w-16 h-16 mx-auto text-yellow-400" fill="currentColor" viewBox="0 0 20 20"><path d="M2 6a2 2 0 012-2h5l2 2h5a2 2 0 012 2v6a2 2 0 01-2 2H4a2 2 0 01-2-2V6z"></path></svg>
                            <p className="mt-2 truncate">{folder.name}</p>
                        </div>
                    ))}
                    {/* Display Images */}
                    {itemsToDisplay.images.map(image => (
                        <div key={image._id} className="p-2 bg-white rounded-lg shadow text-center">
                            <img src={image.imageUrl} alt={image.name} className="w-full h-32 object-cover rounded"/>
